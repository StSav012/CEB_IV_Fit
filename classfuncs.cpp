...
{/*Used to set the base parameters for fitting*/
	...
	par[2]=param[2]; TephPOW=par[2]; ToFit[2]=Fit[2]; //returned power

	par[3]=param[3]; gamma=par[3]; ToFit[3]=Fit[3]; //gap smearing, returned power

	par[4]=param[4]; Vol=par[4]; ToFit[4]=Fit[4]; //volume of absorber in um^3
	...
	par[8]=param[8]; Tc=par[8]; ToFit[8]=Fit[8]; //critical temp of superconductor

	par[9]=param[9]; Rn=par[9]; ToFit[9]=Fit[9]; //total normal resistance, Ohm 

	par[10]=param[10]; Rleak=par[10]; ToFit[10]=Fit[10]; //leakage, Ohm
