...
{ //a procedure which computes the current using approximation
    ...
    i = s * sqrt(2 * PI * a1 / a2 + a3)
        * (1 / (2 * exp((1 - v) / tau) + 1) + 1 / (2 * exp((1 + v) / tau) + 1));
    return i;
}
...
{ //a procedure which computes the cooling power of SIN junction
    ...
    a0 = (1 - v) / tauE;
    a1 = sqrt(2 * PI * tauE) * ((1 - v) / (2 * exp(a0) + 1.28) + 0.5 * tauE / (2 * exp(a0) + 0.64))
         / (exp(-2.5 * (a0 + 2)) + 1);
    if ((v - 1 - tauE) > 0) {
        a2 = sqrt(v * v - 1);
        a3 = 0.5 * (-v * a2 + log(v + a2) + PI * PI * tauE * tauE / 3 * v / a2)
             / (exp(2.5 * (a0 + 2)) + 1);
    }

    b0 = (1 + v) / tauE;
    b1 = sqrt(2 * PI * tauE) * ((1 + v) / (2 * exp(b0) + 1.28) + 0.5 * tauE / (2 * exp(b0) + 0.64))
         / (exp(-2.5 * (b0 + 2)) + 1);
    if ((-v - 1 - tauE) > 0) {
        b2 = sqrt(v * v - 1);
        b3 = 0.5 * (v * b2 + log(-v + b2) - PI * PI * tauE * tauE / 3 * v / b2)
             / (exp(2.5 * (b0 + 2)) + 1);
    }

    c0 = 1 / tau;
    c1 = 2 * sqrt(2 * PI * tau) * (1 / (2 * exp(c0) + 1.28) + 0.5 * tau / (2 * exp(c0) + 0.64))
         / (exp(-2.5 * (c0 + 2)) + 1);
...
{ //computes the Andreev current using exact integral, rectangle method
    ...
    for (l = 0; l <= inf - 1; l++) {
        a0 = tanh((x + v) / (2 * tauE));
        a1 = tanh((x - v) / (2 * tauE));
        a2 = 2 * Wt * sqrt(DT * DT - x * x) / tm
             / (std::pow(2 * Wt * x - x * sqrt(DT * DT - x * x) / DT, 2)
...
long CFoo::CEB_2eq_parallel_lite(void)
{
    ...
    for (j = 1; j <= NV - 1; j++) //next voltage
    {
        ...
        NoiA = (vn * vn
                + std::pow(float(in * (2 * 1e9 / dIdV + Ra) * M / MP), float(2))); //(V/sqrt(Hz))^2

        NEPa = (NoiA) / Sv / Sv; //pW^2/Hz

        //----- NEP SIN approximation ----------------------------

        dI = 2 * EL * abs(I[j]) / std::pow(dIdV * Sv, 2) * 1e9; //pW^2/Hz
...
*/
        ...
        NEPph = sqrt(M * MP * 2 * Pbg * 0 * 1e9 * HPLANCK * 1e-12
        ...
        fprintf(f2,
                "%g\t%g\t%g\t%g\t%g\t%g\t%g\n",
                M * (2 * V[j] + I[j] * Ra * 1e-9),
                sqrt(M * MP * NEPep) * 1e9 * abs(Sv),
                sqrt(M * MP * NEPs) * 1e9 * abs(Sv),
                sqrt(NoiA) * 1e9,
                NEP * 1e9 * abs(Sv),
                NEPph * 1e9 * abs(Sv),
                1e9 * abs(Sv) * sqrt(NEP * NEP - NEPph * NEPph));

        fprintf(f4,
                "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n",
                M * (2 * V[j] + I[j] * Ra * 1e-9),
                MP * 1e-9 * (I[j]),
                sqrt(M * MP * NEPep) * 1e-12,
                sqrt(M * MP * NEPs) * 1e-12,
                sqrt(NEPa) * 1e-12,
                NEP * 1e-12,
                NEPph * 1e-12,
                abs(Sv) * 1e12,
                1e-12 * sqrt(NEP * NEP - NEPph * NEPph));
